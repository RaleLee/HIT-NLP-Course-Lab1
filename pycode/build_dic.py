...
def buildBiDic(lines):
    ...
    for i in range(size):
        ...
        wordlist.insert(0, "<BOS>")
        wordlist.append("<EOS>")
        for j in range(len(wordlist)-1):
            pair = (wordlist[j], wordlist[j+1])
            if pair in dic:
                dic[pair] += 1
            else:
                dic[pair] = 1
...
LMdicpath = "outputs/LMdic.txt"
Bidicpath = "outputs/bidic.txt"
...
def main():
    ...
    LMdic = buildLMDic(lines)
    writeDic(LMdicpath, LMdic)
    Bidic = buildBiDic(lines)
    writeDic(Bidicpath, Bidic)
